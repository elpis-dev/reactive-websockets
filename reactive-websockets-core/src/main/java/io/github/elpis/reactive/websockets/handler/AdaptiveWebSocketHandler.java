...
import io.github.elpis.reactive.websockets.config.flow.BackpressureConfig;
import io.github.elpis.reactive.websockets.config.flow.HeartbeatConfig;
import io.github.elpis.reactive.websockets.config.flow.RateLimitConfig;
...
public abstract class AdaptiveWebSocketHandler extends BaseWebSocketHandler {
  ...
  protected Mono<Void> buildChain(
    ...
    // TODO: Maybe this can re-structured with common decorators for better reusability across
    // different handlers.
   ...
   */
  // TODO: Think about gathering rate limits, backpressure and heartbeat configs during spring bean
  // initialization to separate registries (Then also reusing application.properties values for
  // easier setup).
  // And then just referencing those registries here instead of passing configs around.
  protected String resolveRateLimitIdentifier(final WebSocketSessionContext context) {
    final RateLimit.RateLimitScope scope = rateLimitConfig.getScope();
  ...
  protected <T> Flux<T> applyBackpressure(final Flux<T> flux) {
    ...
    final Backpressure.BackpressureStrategy strategy = backpressureConfig.getStrategy();
  ...
  protected TimeUnit getRateLimitTimeUnit() {
  ...
  protected RateLimit.RateLimitScope getRateLimitScope() {
