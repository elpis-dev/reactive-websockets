...
public class SessionResolver extends SocketApiAnnotationResolver<SessionAttribute> {
    private static final String GET_SESSION_OPTIONAL = """
        final java.util.Optional<org.elpis.reactive.websockets.session.ReactiveWebSocketSession> $L = this.getSessionRegistry().get(context.getSessionId());
        """;

    private static final String GET_SESSION_REQUIRED = """
        final org.elpis.reactive.websockets.session.ReactiveWebSocketSession $L = this.getSessionRegistry().get(context.getSessionId())
        .orElseThrow(() -> new org.elpis.reactive.websockets.exception.WebSocketProcessingException("Cannot find session with id %s", context.getSessionId()));
        """;
    ...
    public CodeBlock resolve(final VariableElement parameter) {
        ...
        final Element optionalType = this.getElements().getTypeElement(Optional.class.getCanonicalName());
        final TypeMirror sessionType = this.getElements().getTypeElement(ReactiveWebSocketSession.class.getCanonicalName())
                .asType();
        ...
        if (this.getTypes().isSameType(this.getTypes().erasure(parameterType),
            ...
            if (!this.isSessionParamValid(parameterType)) {
                throw new WebSocketResolverException("Cannot process @SessionAttribute parameter: bad return type: %s", parameterType);
            }
            ...
            throw new WebSocketResolverException("Only 'ReactiveWebSocketSession' type is supported for " +
                    "@SessionAttribute. Found '%s'", parameterType.toString());
        }
    }
    ...
    private boolean isSessionParamValid(final TypeMirror type) {
        if (type instanceof DeclaredType declaredReturnType) {
            ...
            throw new WebSocketResolverException("Cannot process @SessionAttribute parameter: bad return type: %s",
                    type.toString());
        }
    }
