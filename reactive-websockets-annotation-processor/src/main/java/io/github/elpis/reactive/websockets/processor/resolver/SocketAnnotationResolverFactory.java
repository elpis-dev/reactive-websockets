...
public class SocketAnnotationResolverFactory {
  private static final Map<
          Class<? extends Annotation>, BiFunction<Elements, Types, SocketApiAnnotationResolver<?>>>
      BASE_SUPPORTED_RESOLVERS =
          Map.of(
  ...
  private static final Map<
          Class<? extends Annotation>, BiFunction<Elements, Types, SocketApiAnnotationResolver<?>>>
      SUPPORTED_RESOLVERS = new ConcurrentHashMap<>();

  private SocketAnnotationResolverFactory() {}

  public static Optional<BiFunction<Elements, Types, SocketApiAnnotationResolver<?>>> findResolver(
      final VariableElement element) {
    final List<BiFunction<Elements, Types, SocketApiAnnotationResolver<?>>> annotationsFiltered =
        Stream.concat(
                BASE_SUPPORTED_RESOLVERS.entrySet().stream(),
    ...
    if (annotationsFiltered.size() > 1) {
      final String failedAnnotations =
          annotationsFiltered.stream()
              .map(annotation -> "@" + annotation.getClass().getName())
              .collect(Collectors.joining(","));

      throw new WebSocketProcessorException(
          "Ambiguous WebSocket annotations found %s. " + "Only one declared annotation is legal",
          failedAnnotations);
    }

    return annotationsFiltered.stream().findFirst();
  }
}
