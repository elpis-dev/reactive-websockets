package io.github.elpis.reactive.websockets.config.flow;
...
public record BackpressureConfig(Optional<BackpressureConfigData> config) {
   ...
   */
  public static BackpressureConfig of(Backpressure.BackpressureStrategy strategy, int bufferSize) {
  ...
  public Backpressure.BackpressureStrategy getStrategy() {
    return config
        ...
        .orElse(Backpressure.BackpressureStrategy.BUFFER);
  }
